...
sampsizes <- matrix(c(1:D,Nd),ncol=2)
area_size <- matrix(c(area,weightsaug),ncol=2)
...
propfinMC.Dir <-matrix(0,nr=D,nc=nMC)
gapfinMC.Dir <-matrix(0,nr=D,nc=nMC)
propfinMC.FH <-matrix(0,nr=D,nc=nMC)
gapfinMC.FH <-matrix(0,nr=D,nc=nMC)
...
plot(1:D,EmpiricalMSEgapPre,type="n",xlab="Area",ylab="MSE Poverty incidence (x10000)",ylim=c(0,b))
